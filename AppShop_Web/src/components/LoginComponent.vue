...
const login = () => {
  //console.log(form)
  if (form.password == '' || form.telephone == '') {
    ...
    axios.post('/api/login',
      {
        ...
        if (res.data.code == 200) {
          ...
          else if (res.data.data.User.tag == 10) {
          ...
          else if (res.data.data.User.tag == 1) {
            router.push('/notmanager')
            ElMessage.warning('您还不是管理员！请联系开发者！')
          }
            store.dispatch('id_save', res.data.data.User.telephone)
            store.dispatch('token_save', res.data.data.token)
            //store.dispatch('type_save', res.data.data.type)
